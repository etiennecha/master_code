...
In this paper, the richness of the data allows to study competition across local markets in an unprecedented way.\footnote{\cite{LAC02} and \cite{ZHA06} are strongly limited both in terms of store number and product sample. In \cite{PER15}, the size and structure of the consumer panel, meant to be representative of France as a whole, do not allow to measure dispersion within local markets.}. The core of the paper focuses on millions of prices collected from nearly 2,300 large supermarkets by a price comparison websites in March 2015. 

The first part of the paper is dedicated to quantifying price dispersion and discussing the extent to which obtained measures are likely to reflect imperfect consumer information. Prices are found to depend strongly on store chain affiliation, and are poorly explained by observable market characteristics, in particular one-size-fits-all measures of market power. One chain is observed to virtually operate a uniform pricing policy. Store price comparisons yield more volatile results when stores are separated by a higher distance, which supports a relation between price dispersion and consumer search costs. The second part of the paper investigates the relation between price dispersion, as a measure of


The analysis indeed relies on millions of prices records collected from . While 

sused by a price comparison website to perform comparisons between n.


The French grocery store chain Leclerc operates a price comparison website which allows to compare each of its stores with a selection of competitors, and performs chain comparisons at the national level. A large price cross section collected from the website, including over , is used to investigate price levels and dispersion across local markets in an unprecedented way. 

 At the market level, the lowest and the highest prices of a given product are separated by an average 17\% difference. Controlling for store fixed effects only reduces this gap to 10\%. Price dispersion is loosely related to observable market characteristics, but increases significantly with an index of market prices. These findings are consistent with stores generally using noisier price strategies to extract higher profits when consumer loyalty or search costs allow them to do so.
