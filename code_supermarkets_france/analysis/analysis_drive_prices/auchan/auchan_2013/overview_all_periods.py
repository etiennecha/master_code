...
# todo
# see if can include 2013-12-12 to 2014-01-29
# would allow c. 1 month overlap with Carrefour Voisins le Bx
# otherwise can compare Auchan Velizy vs Plaisir
# (promo strat broadly since not capture very well)
...
# after 2013-05-13: no distinction between loyalty and promo
...
df_master.rename(columns = {'title': 'product'},
                 inplace = True)
df_master_s = df_master[df_master['store'] == 'velizy']
...
ax = df_prices_sc[u"Président coulommiers 350g"].plot()
ax.yaxis.set_major_formatter(plt.FormatStrFormatter('%.2f'))
plt.show()

ax = df_prices_sc[u"33 Export bière 4,5° -24 x 25cl"].plot()
ax.yaxis.set_major_formatter(plt.FormatStrFormatter('%.2f'))
plt.show()
...
df_su_s = pd.concat([se_nb_prod, se_nb_promo, se_nb_loyalty],
                    axis = 1,
                    keys = ['nb_prod', 'nb_promo' 'nb_loyalty'])
