...
if __name__=="__main__":
  ...
  pd_df_stats_apps = pd.read_csv(path_data + folder_stats_apps + r'\Bretagne.csv', dtype = str)
  ...
  ls_pbm_ids = [id_station for id_station in ls_ids_stats_apps if id_station not in master_info]
  ...
  for file_name in files_source_data:
    ...
    ls_remaining_pbms = [id_station for id_station in ls_remaining_pbms if id_station not in ls_dict_info_old[-1]]
  ...
  for id_station in ls_ids_stats_apps:
    if id_station in master_info:
      ...
      elif id_station in pd_df_rls.index and pd_df_rls.ix[id_station]['latDeg']:
        gps = [pd_df_rls.ix[id_station]['latDeg'], pd_df_rls.ix[id_station]['longDeg']]
  ...
  for id_station in ls_ids_stats_apps:
    ...
    elif id_station in pd_df_rls.index and pd_df_rls.ix[id_station]['CodeCom']:
      dict_new_info_stats_apps[id_station]['code_geo'] = pd_df_rls.ix[id_station]['CodeCom']
    else:
      print id_station, 'no insee code'
  dict_new_info_stats_apps['29000008']['code_geo'] = master_price['dict_info']['29000010']['code_geo']
  
  # Check that dict_new_info_stats_apps complete and generate DataFrame
  ...
  # Rebrand services, Merge DataFrames, Output to csv
  ...
  pd_df_stats_apps.to_csv(path_data + folder_stats_apps + r'\Bretagne_updated.csv', index=False)