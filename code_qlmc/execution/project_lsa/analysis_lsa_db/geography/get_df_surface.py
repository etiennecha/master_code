...
print u'Test with commune', df_com['commune'].iloc[0]
...
se_nb_stores = df_lsa['Code INSEE ardt'].value_counts()
se_surf_stores = df_lsa[['Surf Vente', 'Code INSEE ardt']].groupby('Code INSEE ardt').agg(sum)['Surf Vente']
...
def wgted_avg_and_std(values, weights):
  average = np.average(values, weights=weights)
  variance = np.average((values-average)**2, weights=weights)
  return (average, np.sqrt(variance))

# Nb of stores by com weighted by nb of households
print '\nAvg and std nb stores weighted by pop:',\
      wgted_avg_and_std(df_com['nb_stores'], df_com['nb_households'])

import weighted
for quantile in [0.25, 0.5, 0.75]:
  print 'Quantile', quantile, ':', weighted.quantile(df_com['nb_stores'],
                                                     df_com['nb_households'],
                                                     quantile)

# Nb of stores by com weighted by nb of households
print '\nAvg and std surf weighted by pop:',\
      wgted_avg_and_std(df_com['surf'], df_com['nb_households'])
