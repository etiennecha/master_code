...
Pairs involving supermarkets and discounters typically display large percentages of similar prices over time. Observations of strong price alinement raise the question of convergence to equilibrium and potential collusion. In order to gain some understanding of price dynamics, for each pair, we count the number of times each gas station matches its competitors' price. A binomial test of equality often leads to reject the hypothesis that both gas station are equally likely to match the other's price. On a technical note, it must me noted that with daily price records, it is often the case that competitors with similar price levels simultaneously set the same price on a given day. Such observations are not taken into account in our approach.
