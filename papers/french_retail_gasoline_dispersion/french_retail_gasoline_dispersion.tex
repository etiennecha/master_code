...
Data provide several examples of high rank reversal statistics unrelated to consumer lack of information. The conversion by Total S.A. of Total gas stations to its discount brand, Total Access, generates many observations with spuriously high rank reversals. Indeed, rank reversals are recorded due to a one-off change in the pricing policy of converted gas stations, and competitors which virtually do not react (\cite{CHA16}). Figure XXX provides an example

More generally, such an issue may arise whenever a shock affects a gas station differently from the way it affects its closest competitors, so that relative competitive advantages change over time. In order to reduce the potential influence of this issue, gas stations which are found to implement a significant change in pricing policy over the period are excluded, and pairs for which the average length of rank reversals exceeds 20 days are dropped.
