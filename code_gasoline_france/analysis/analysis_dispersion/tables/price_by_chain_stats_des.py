...
df_sub_all = df_sub.copy()
# df_sub = df_sub[df_sub['nb_brand'] >= 50]


for group_type in df_sub['group_type'].unique():
  df_sub.loc[(df_sub['nb_brand'] <= 50) &\
             (df_sub['group_type'] == group_type),
...
for group_types in ls_group_types:
  se_gt= df_sub_all[df_sub_all['group_type'].isin(group_types)]\
                   ['price_at'].describe(percentiles = ls_pctiles)
  ls_se_gt.append(se_gt)
  ls_gt_title.append('&'.join(group_types))

df_desc_agg = pd.concat(ls_se_gt, keys = ls_gt_title, axis = 1)
df_desc_agg = df_desc_agg.T
df_desc_agg['Q75/Q25'] = df_desc_agg['75%'] / df_desc_agg['25%']
df_desc_agg['Q90/Q10'] = df_desc_agg['90%'] / df_desc_agg['10%']
df_desc_agg['count'] = df_desc_agg['count'].astype(int)

print()
print(u'Price distribution by agg group type:')
print(df_desc_agg.to_string(float_format = '{:,.2f}'.format))
