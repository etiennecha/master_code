...
Price records of the 2007-2012 period were made available by Qlmc in pdf files. They were thus downloaded and processed to build a database.

\subsection{Overview of 2007-2012 data}
...
Since 2013, the website does not make data easily available (Footnote: Pdf files exist for price records of May 2014 but are not used in the current version of the paper). In March 2015, each store comparison available on the website was thus crawled to build a price database, which is therefore bound to be significantly smaller than original one. Indeed, only stores which compete with a Leclerc outlet are included, and for each pair of stores, prices are provided only for goods available at both store.
