...
for i, row in df_com.iterrows():
  ...
  df_com.loc[i, 'avail_surf'] = df_lsa['wgt_surf'].sum()
  
  # Groupe col: diff for each indpt store?
  df_rgps = df_lsa[['Groupe', 'wgt_surf']].groupby('Groupe').agg([sum])['wgt_surf']
  df_rgps['market_share'] = df_rgps['sum'] / df_rgps['sum'].sum()
  df_com.loc[i, 'hhi'] = (df_rgps['market_share']**2).sum()
  
  df_rgps.sort('sum', ascending = False, inplace = True)
  df_com.loc[i, 'CR1'] = df_rgps['sum'][0:1].sum() / df_rgps['sum'].sum()
  df_com.loc[i, 'CR2'] = df_rgps['sum'][0:2].sum() / df_rgps['sum'].sum()
  df_com.loc[i, 'CR3'] = df_rgps['sum'][0:3].sum() / df_rgps['sum'].sum()

  for store_type in ['H', 'X', 'S']:
    store_ind = df_lsa['dist'][df_lsa['Type_alt'] == '%s' %store_type].argmin()
    df_com.loc[i, '%s_ens' %store_type] = df_lsa.loc[store_ind, 'Enseigne']
    df_com.loc[i, '%s_dist' %store_type] = df_lsa.loc[store_ind, 'dist']
