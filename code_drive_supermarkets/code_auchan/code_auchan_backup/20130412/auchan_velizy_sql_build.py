...
for row in cursor.execute("SELECT product_total_price FROM auchan_velizy WHERE product_title = ?", where_condition):
...
# keep non duplicates only (regardless of which category they belong to here...)
period = (1,)
cursor.execute("DELETE FROM auchan_velizy WHERE auchan_velizy.period = ?\
                                          AND EXISTS(SELECT 1 FROM auchan_velizy t2\
                                                       WHERE auchan_velizy.product_title = t2.product_title\
                                                       AND auchan_velizy.rowid > t2.rowid)", period)
