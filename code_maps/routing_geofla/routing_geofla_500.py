...
path_dir_geofla_km = os.path.join(path_data, 'data_maps', 'ROUTE%s_WGS84' %geofla_km)

ls_files_geofla_km = [('TRONCON_ROUTE', 'routes'),
                      ('NOEUD_ROUTIER', 'noeuds'),
                      ('NOEUD_COMMUNE', 'communes')]

for file_orig, file_dest in ls_files_geofla_km:
  m_fra.readshapefile(os.path.join(path_dir_geofla_km, file_orig),
                      file_dest,
                      color = 'none',
                      zorder = 2)
...
# Base files for which ID_RTEXXX is a key
ls_km_main = [('routes', m_fra.routes, m_fra.routes_info),
              ('noeuds', m_fra.noeuds, m_fra.noeuds_info),
              ('communes', m_fra.communes, m_fra.communes_info)]

dict_km_main = {}
for field, ls_field_geo, ls_field_info in ls_km_main:
  dict_km_main[field] = {}
  # position i should not be used... just bu
  for i, (geo, info) in enumerate(zip(ls_field_geo, ls_field_info)):
    dict_km_main[field][info['ID_RTE%s' %geofla_km]] = [geo, info, i]
...
#         os.path.join(path_built, 'dict_%s_route_nodes.json' %geofla_km))

##dict_km_node_routes = dec_json(os.path.join(path_built, 'dict_%s_node_routes.json' %geofla_km))
##dict_km_route_nodes = dec_json(os.path.join(path_built, 'dict_%s_route_nodes.json' %geofla_km))
##
### JSON stores keys as string: convert back?
##for k,v in dict_km_node_routes.items():
##  dict_km_node_routes[int(k)] = v
##  del(dict_km_node_routes[k])
##
##for k,v in dict_km_route_nodes.items():
##  dict_km_route_nodes[int(k)] = v
##  del(dict_km_route_nodes[k])
##
