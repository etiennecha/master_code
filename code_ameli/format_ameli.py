...
# Clean (0) name-address-phone component
ls_formatted = []
for id_physician, ls_physician in dict_physicians.items():
  ...
  ls_formatted.append([ls_name, phone, ls_address])

# Clean (1) multiple location component (TODO)

# Clean (2) info component (clean already?)
...
for id_physician, ls_physician in dict_physicians.items():
  if ls_physician[3]:
    for service in ls_physician[3]:
      ls_services.append(tuple(service[0][0]))
...
for id_physician, ls_physician in dict_physicians.items():
  ...
  ls_all_physician_services.append(ls_physician_services)
# Seems ok: stats desc about service available and price (vs. sec 1. sec.2...)
ls_consultations = []
ls_consultations_prices = []
for i, ls_services in enumerate(ls_all_physician_services):
  for service in ls_services:
    if service[0] == u'Consultation' and service[2]:
      ls_consultations.append((i, service[2]))
      if len(service[2]) == 1:
        ls_consultations_prices.append(re.findall(u'[0-9]{2,3}\u20ac', service[2][0][1]))
      else:
        ls_consultations_prices.append(re.findall(u'[0-9]{2,3}\u20ac', service[2][1][1]))
