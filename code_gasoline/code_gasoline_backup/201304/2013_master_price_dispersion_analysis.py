...
import sys
sys.path.append(r'C:\Users\etna\Desktop\20120226_final_gasoline_project\scripts')
import ls as ls
...
machine_path = r'C:\Users\etna\Desktop\20120226_final_gasoline_project'
...
def get_master_price_change_frequency(master_price):
  ...
  for list_prices in master_price['list_indiv_prices']:
    price_array_today = np.array([x if x not in [None, ' --'] else np.nan for x in list_prices][:-1])
    price_array_tomorrow = np.array([x if x not in [None, ' --'] else np.nan for x in list_prices][1:])
    changes = price_array_tomorrow - price_array_today
    nb_same_price = ((np.float64(0) == changes)).sum()
    nb_decrease = ((np.float64(0) > changes)).sum()
    nb_increase = ((np.float64(0) < changes)).sum()
    avg_decrease = np.mean(changes[np.float64(0) > changes])
    avg_increase = np.mean(changes[np.float64(0) < changes])
    list_price_change_frequency.append([nb_same_price, nb_decrease, nb_increase, avg_decrease, avg_increase])
...
def get_master_pair_price_dispersion(list_competitor_pairs, master_price, km_bound):
  # add the average spread conditional on rank being reversed
...
# 1/ Change frequency and value analysis

test_1 = get_station_price_change_frequency('1500007', master_price_gas_stations)
test_2 = get_master_price_change_frequency(master_price_gas_stations)

# 2/ Price dispersion analysis (and data display)
...
# get_plot_prices([get_rank(id, master_price_gas_stations) for id in station_price_dispersion[0]], master_price_gas_stations)
# print_list_of_lists_from_ranks([get_rank(id, master_price_gas_stations) for id in station_price_dispersion[0]], master_price_gas_stations, 10)

# 3/ Regression framework
# Let's start with pypeR (see if not too heavy... else compare ls.OLS with STATA)
# In any case: check that formats make it easy to build variable array/matrixes and print in txt file (for STATA)

# First: average price per period
...
"""
matrix_pair_pd = np.array([pd_tuple for pd_tuple in master_pair_price_dispersion if pd_tuple[1] < 3])
...
print ls.OLS(rank_reversals, distance, constant = 1)
print ls.OLS(spread_std, distance, constant = 1)
"""

# header_master_pair_pd = 'distance, rank_reversals, spread_std'
# np.savetxt(r'C:\Users\etna\Desktop\Stata_np_files\master_pair_pd.txt', matrix_pair_pd, fmt = '%.5f', delimiter = ',', header = header_master_pair_pd)

# Regression of market price dispersion on nb competitors

"""
matrix_master_pd = np.array([0, 0, 0, 0, 0, 0])
for individual in master_market_price_dispersion:
  ...
  if np.all(~np.isnan(temp_matrix_master_pd)):
    matrix_master_pd = np.vstack([matrix_master_pd, temp_matrix_master_pd.T])
matrix_master_pd = matrix_master_pd[1:,:]
...
print ls.OLS(std_prices, nb_competitors, constant = 1)
print ls.OLS(coeff_var_prices, nb_competitors, constant = 1)
print ls.OLS(range_prices, nb_competitors, constant = 1)
print ls.OLS(gain_search, nb_competitors, constant = 1)

print ls.OLS(std_prices, prices_and_nb_competitors, constant = 1)
print ls.OLS(coeff_var_prices, prices_and_nb_competitors, constant = 1)
print ls.OLS(range_prices, prices_and_nb_competitors, constant = 1)
print ls.OLS(gain_search, prices_and_nb_competitors, constant = 1)
"""
# header_master_market_pd = 'nb_competitors, prices, std_prices, coeff_var_prices, range_prices, gain_search'
# np.savetxt(r'C:\Users\etna\Desktop\Stata_np_files\matrix_master_pd_5.txt', matrix_master_pd, fmt = '%.5f', delimiter = ',', header = header_master_market_pd)

# Cleaning prices to recompute price dispersion with cleaned prices

dico_brands = dec_json(machine_path + r'\\' + brand_folder + r'\\dico_brands')
...
for coeff in list_coeffs:
  master_coeffs.append([coeff for i in range(0, len(master_price_gas_stations['list_indiv_prices'][0]))])
matrix_coeffs = np.array(master_coeffs, dtype = np.float64)
matrix_price_cont = master_np_prices - matrix_coeffs

master_price_gas_stations['list_indiv_prices'] = matrix_price_cont
...
# np.savetxt(r'C:\Users\etna\Desktop\Stata_np_files\master_pair_pd_cont.txt', master_pair_price_dispersion_cont_print, fmt = '%.5f', delimiter = ',', header = header_master_pair_pd_cont)
...
matrix_master_pd_cont = np.array([0, 0, 0, 0, 0, 0])
for individual in master_market_price_dispersion_cont:
  array_nb_competitors = np.ones(len(individual[2])) * individual[1]
  temp_matrix_master_pd_cont = np.vstack([array_nb_competitors, period_mean_prices, individual[2:]])
  matrix_master_pd_cont = np.vstack([matrix_master_pd_cont, temp_matrix_master_pd.T])
matrix_master_pd_cont = matrix_master_pd_cont[1:,:]
# header_master_market_pd = 'nb_competitors, mean_period_price, std_price, coeff_var_price, range_price, gain_from_search'
# np.savetxt(r'C:\Users\etna\Desktop\Stata_np_files\master_market_pd_cont_5.txt', matrix_master_pd_cont, fmt = '%.5f', delimiter = ',', header = header_master_market_pd)

"""
master_id_array_np = np.array('', dtype = str)
master_period_array_np = np.array([0], dtype = np.int64)
master_price_np = np.array([0], dtype = np.float64)
master_brand_np = np.array('', dtype = str)
for i in range(0,len(master_price_gas_stations['list_indiv_prices'])):
  price_array = np.array(master_price_gas_stations['list_indiv_prices'][i], dtype = np.float64)
  id = master_price_gas_stations['list_ids'][i]
  id_array = np.array([id for i in range(0, len(price_array))], dtype = str)
  period_array = np.array([i for i in range(0, len(price_array))], dtype = np.int64)
  brand = dico_brands[get_str_no_accent(master_price_gas_stations['dict_general_info'][id]['brand_station'][0][0]).upper()][1]
  brand_array = np.array([brand for i in range(0, len(price_array))], dtype = str)
  master_price_np = np.hstack([master_price_np, price_array])
  master_id_array_np = np.hstack([master_id_array_np, id_array])
  master_period_array_np = np.hstack([master_period_array_np, period_array])
  master_brand_np = np.hstack([master_brand_np, brand_array])
master_long = np.vstack([master_id_array_np, master_period_array_np, master_price_np, master_brand_np]).T
master_long = master_long[1:,:]
"""
