...
\end{align*}
where $\text{Product}_i$ is a product effect and $\text{Store}_j$ is a store effect. The coefficient $\text{Product}_i$ associated with a dummy variable equal to $1$ for products carried by store $i$ provides information regarding the price level of store $i$ compared to a reference store. The residual variation $\epsilon_{ij}$ is approximately equal to the percentage deviation of a store's price from the geometric mean price in the reference group. The residuals average to zero for each product and each store, but a measure such as the standard deviation of residuals for a given store provides a proxy for price dispersion at the store level.
