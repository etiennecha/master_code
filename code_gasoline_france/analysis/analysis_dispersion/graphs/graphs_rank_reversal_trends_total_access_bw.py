...
df_pairs = df_pairs[df_pairs['cat'] == 'all'].copy()
...
df_pair_comp_ta =\
  df_pair_comp[(df_pair_comp['brand_last_1'] == 'TOTAL_ACCESS') |\
...
df_rr_ta = df_rr.ix[df_pair_comp_ta['tup_ids'].values]
df_rr_nta = df_rr.ix[df_pair_comp_nta['tup_ids'].values]

zero = np.float64(1e-10)
ls_df_rrs_su = []
for df_rrs_temp in [df_rr, df_rr_nd, df_rr_ta]:
...
ls_loop_rr_temp = [['All pairs', ls_df_rrs_su[0]],
                   ...
                   ['With Total Access', ls_df_rrs_su[2]]]
...
plt.savefig(os.path.join(path_dir_built_dis_graphs,
                         dir_graphs,
                         'macro_rank_reversals_ta_le_{:.0f}c.png'.format(diff_bound*100)),
            bbox_inches='tight')
plt.close()
