...
# FILTER: ILE FRANCE
df_com = df_com[df_com['reg_name'] == u'ILE-DE-FRANCE']
df_dpt = df_dpt[df_dpt['reg_name'] == u'ILE-DE-FRANCE']
# does not keep those around but not inside Ile-de-France
df_ta = df_ta[df_ta['reg'] == u'Ile-de-France']

df_ta_converted = df_ta[df_ta['treatment_0'].isin([5,6])]
df_ta_total = df_ta[(df_ta['treatment_0'] == 1) & (df_ta['group'] == 'TOTAL')]
df_ta_comp = df_ta[(df_ta['treatment_0'] == 1) & (df_ta['group'] != 'TOTAL')]

df_control = df_ta[(df_ta['treatment_0'] == 3)]
df_out = df_ta[(df_ta['treatment_0'] == 4)]
...
ls_display_dfs = [['Total Access', df_ta_converted, 'darkorange', 3, 1],
                  ['Total', df_ta_converted, 'r', 2, 0.4],
                  ['Competitors', df_ta_comp, 'b', 2, 0.4],
                  ['Control', df_control, 'g', 2, 0.4],
                  ['Out', df_out, 'k', 2, 0.4]]
...
ls_l = []
for label, df_temp, fcolor, zorder, alpha in ls_display_dfs:
  ls_l.append(ax1.scatter([store.x for store in df_temp['point']],
                          [store.y for store in df_temp['point']],
                          s = 20, marker = 'o', lw=0,
                          facecolor = fcolor, edgecolor = 'w',
