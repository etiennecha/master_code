...
df_dd_control = pd.DataFrame(df_prices_ht[ls_control_ids].mean(1),
                             df_prices_ht.index, ['price'])
...
for id_station, ls_ta_comp in dict_ls_ta_comp.items():
  if df_info.ix[id_station]['brand_0'] not in ['ELF', 'TOTAL', 'TOTAL_ACCESS', 'ELAN']:
    ...
    if (ls_ta_comp) and (ls_ta_comp[0][1] <= 3):
      ...
      df_dd_comp = pd.DataFrame(df_prices_ht[id_station].values,
                                df_prices_ht.index, ['price'])
...
reg_dd_res = smf.ols('price ~ C(time) + %s + treatment' %str_station_fe,
                     data = df_dd,
                     missing = 'drop').fit()
