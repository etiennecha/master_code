...
df_stores = pd.read_csv(os.path.join(path_built_csv, 'df_stores_final.csv'),
                        dtype = {'c_insee' : str,
                                 'id_lsa' : str},
                        encoding = 'utf-8')
# Rename chains to have similar chains as on qlmc website
...
ls_keep_store_ids = list(df_prices['store_id'].unique())
df_stores = df_stores[df_stores['store_id'].isin(ls_keep_store_ids)]
...
dict_chain_ids = {}
for row_i, row in df_stores[~df_stores['id_lsa'].isnull()].iterrows():
...
for i in [1, 2]:
  df_close_pairs['store_id_{:d}'.format(i)] =\
    df_close_pairs.apply(\
      lambda row: df_stores.ix[row['id_lsa_{:d}'.format(i)]]['store_id'],
      axis = 1)
...
for row_i, row in df_close_pairs.iterrows():
  ...
  df_store_1  = df_chain[df_chain['store_id'] == row['store_id_1']]
  df_store_2  = df_chain[df_chain['store_id'] == row['store_id_2']]
  ...
  if price_2 - price_1 > 10e-4:
    ...
    mean_compa = 0
    winner_compa = 'draw'
  ls_rows_compa.append((row['store_id_1'],
                        row['id_lsa_1'],
                        row['store_id_2'],
                        row['id_lsa_2'],
                        row['store_chain'],
