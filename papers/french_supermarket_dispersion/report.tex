...
\title{French grocery store prices: Evidence from a price comparison website\ \\ \ \\Working paper}
...
The French supermarket chain Leclerc operates a price comparison website which allows to compare each of its stores with some local competitors, and provides aggregate chain comparisons at the national level. This papers uses price data collected from the website to investigate static and dynamic price dispersion across French grocery stores. Results show that chain level comparisons are generally of little informative value given the heterogeneity at the store level. Furthermore, store comparisons tend to be highly sensitive to the basket of products and vary from one period to the next.
