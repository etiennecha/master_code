...
try:
  patch = PolygonPatch(my_poly) # check if connected (really?)
  shapely_polygon = my_poly
except:
  shapely_polygon = my_poly.convex_hull # if not connected
...
paris_poly_gps = [' '.join(map(lambda x: "{:.4f}".format(x),\
    m_fra(x, y, inverse = True))[::-1]) for x,y in paris_poly]

paris_poly_str = u'paris\n1\n' +\
                    u''.join([u'    %s\n' %x for x in paris_poly_gps])+\
                    u'END'

# todo: output to text file and try with osmosis
