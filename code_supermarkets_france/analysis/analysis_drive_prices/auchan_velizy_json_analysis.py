...
def analyse_price_series(list_price_series, master_price):
  ...
  ls_none = []
  ls_ls_dilettante = []
  ls_missing_day_ind = [master_price['ls_dates'].index(date) for date in master_price['ls_missing_dates']]
  for indiv_index, price_series in enumerate(list_price_series):
    ls_price_durations = []
    ls_bad_reporting = []
    for group in itertools.groupby(iter(range(len(price_series))), lambda x: price_series[x]):
...
master = dec_json(path_data + folder_built_auchan_velizy_json + r'/master')
dict_dpt_subdpts = dec_json(path_data + folder_built_auchan_velizy_json + r'/dict_dpt_subdpts')
...
for product, product_info in master['dict_info'].iteritems():
  ls_product_subdpts = [subdpt for (ls_subdpts, ls_subdpts_ind) in product_info['sub_department']\
...
ls_ls_angles = [get_field_as_list(product, 'product_flag', master) for product in master['ls_ids']]
ls_ls_promo = [[1 if angle == u'angle flag_promo' else 0 for angle in ls_angles] for ls_angles in ls_ls_angles]
...
ls_ls_amounts = [get_field_as_list(product, 'product_promo_amount', master) for product in master['ls_ids']]
ls_ls_amounts = [[ls_ls_amounts[i][j] if price else None for j, price in enumerate(ls_prices)]\
                    for i, ls_prices in enumerate(master['ls_ls_prices'])]
ls_ls_types = [get_field_as_list(product, 'product_promo_type', master) for product in master['ls_ids']]
...
ls_ls_vignettes = [get_field_as_list(product, 'product_promo_vignette', master) for product in master['ls_ids']]
ls_ls_vignettes = [[ls_ls_vignettes[i][j] if price else None for j, price in enumerate(ls_prices)]\
                    for i, ls_prices in enumerate(master['ls_ls_prices'])]
...
print pd_df_promo_period[pd_df_promo_period['ls_promo']==1].count()
print pd_df_promo_period['dpt'][pd_df_promo_period['ls_promo']==1].value_counts()
print pd_df_promo_period['subdpt'][pd_df_promo_period['ls_promo']==1].value_counts()
pd.set_option('display.max_columns', 10)
pd.set_option('display.width', 200)
# d_df_promo_period = pd_df_promo_period[['dpt', 'subdpt','ls_promo']]
# print pd_df_promo_period[pd_df_promo_period['ls_promo']==1].sort(columns = ['dpt', 'subdpt']).to_string()
...
pd_df_all_dpt_products = pd.concat(ls_df_dpt_products, axis=1, keys = pd_df_price.index).T
...
for date_ind in pd_df_price.index:
  pd_df_period = pd_pd_master.major_xs(date_ind).T
  pd_df_period['ls_price'] = pd_df_period['ls_price'].astype(np.float32)
  ls_df_subdpt_products.append(pd_df_period['subdpt'][~np.isnan(pd_df_period['ls_price'])].value_counts())
pd_df_all_subdpt_products = pd.concat(ls_df_subdpt_products, axis=1, keys = pd_df_price.index).T
...
# pd_df_all_dpt_promo[[u'Produits Frais', u'boissons', u'épicerie', u'Surgelés',u'Fruits & Légumes']].plot()
# plt.show()
...
pd_df_all_subdpt_promo = pd.concat(ls_df_subdpt_promo, axis =1, keys = pd_df_price.index).T
...
pd_recap = pd.concat([pd_promo_percent,
                      ...
                      pd_df_all_subdpt_products.sum()],axis=1, join='outer', keys = ['percent', 'promo', 'tot'])
...
ls_ls_promo_nogap = [[1 if angle == u'angle flag_promo' else 0 for angle in ls_angles] for ls_angles in ls_ls_angles]
ls_ls_vignettes_nogap = [get_field_as_list(product, 'product_promo_vignette', master) for product in master['ls_ids']]
ls_ls_amounts_nogap = [get_field_as_list(product, 'product_promo_amount', master) for product in master['ls_ids']]
...
for i, ls_amounts in enumerate(ls_ls_amounts_nogap):
  ...
  ls_promo_durations = []
  for group in itertools.groupby(iter(range(len(ls_amounts))), lambda x: ls_amounts[x]):
    ls_promo_durations.append((group[0], list(group[1])))
  ls_promo_durations = [(promo, ls_days) for (promo, ls_days) in ls_promo_durations if promo and promo != u'0']
...
for i, ls_vignettes in enumerate(ls_ls_vignettes_nogap):
  ...
  ls_promo_durations_2 = [(promo, ls_days) for (promo, ls_days) in ls_promo_durations_2 if promo]
...
# TODO: fill gaps if length < 5 (?) and prices before/after are equal (product not available at drive but..)
# TODO: gaps not filled: price durations before/after not very reliable... (minor mistake overall?)

# TODO: function to analyse price series: beginning, end, nb of prices, durations, size of changes...
# TODO: on one line by subdpt/dpt: nb items, nb of items with sales, nb with full spell, nb with price chges

# ###########
# DEPRECATED
# # ###########

# # Rough idea of sales used (first sales if any for each product)
# ls_percentage = []
# for i, ls_amounts in enumerate(ls_ls_amounts):
  # for j, amount in enumerate(ls_amounts):
    # if amount and amount != u'0':
      # if ls_ls_types[i][j] == u'%':
        # percentage = float(amount)/100
      # else:
        # percentage = float(amount)/float(master['ls_ls_prices'][i][j])
      # ls_percentage.append((i,j,percentage))
      # break

# ls_group_products = []
# ls_sales_products = []
# for product, product_info in master['dict_info'].iteritems():
  # ls_product_dpts = [info for (ls_info, ls_info_ind) in product_info['department'] for info in ls_info]
  # ls_product_subdpts = [info for (ls_info, ls_info_ind) in product_info['sub_department'] for info in ls_info]
  # ls_product_promo_vignettes = [info for (info, info_ind) in product_info['product_promo_vignette'] if info]
  # if 'Boucherie' in ls_product_subdpts:
    # ls_group_products.append(product)
    # if ls_product_promo_vignettes:
      # ls_sales_products.append([product, ls_product_promo_vignettes])