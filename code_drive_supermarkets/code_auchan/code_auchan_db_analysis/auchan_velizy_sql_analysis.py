...
def enc_stock_json(file_to_encode, file_path):
...
def get_relative_frequency(dict_absolute_frequency):
  ...
  for k, v in dict_absolute_frequency.iteritems():
    dict_relative_frequency[k] = np.round([v/float(nb_observations)], decimals = 2) # need [] else does not work
...
# TODO: Numerous (unjustified) duplicates until script changed on april 11, 2013 (NOT IN DB SO FAR...)
# TODO: Since then, duplicates are products that are listed twice or more by auchan
# TODO: Deal with products belonging to different dpts/sub-depts using clean date data
# TODO: Check how many doubles this leaves (can be dealt with manually?)
# SOLUTION: Remove records of (products, sub-dpt) which are not present after april 11 while product is present

# Due to Memory problems... rather correct raw files from first correct file (save script/old files just in case)
# Some problems may remain if products no more exist as of april 11, 2013
...
cursor.execute("CREATE TABLE auchan_velizy_test AS\
                            SELECT * FROM auchan_velizy table1 INNER JOIN \
                                  (SELECT product_title, sub_department FROM auchan_velizy WHERE period >= 50) table2 \
...
# get the prices at all (available) periods of a given product (avoids duplicates, slow version a priori)
list_prices_cond = [row for row in cursor.execute("SELECT product_total_price, period FROM auchan_velizy \
                                                   WHERE product_title = ? AND rowid IN \
												  (SELECT MIN(rowid) FROM auchan_velizy t \
												   GROUP BY product_title, period)", where_condition)]

# alternative, seems faster (?)
list_prices_cond_2 = [row for row in cursor.execute("SELECT DISTINCT product_total_price, period \
                                                     FROM auchan_velizy WHERE product_title = ? \
													 ORDER BY MAX(period) ASC", where_condition)]
...
for sub_department in list_sub_departments[0:1]:
  ...
  list_products_cond = [row[0] for row in cursor.execute("SELECT DISTINCT product_title \
                                                          FROM auchan_velizy \
                                                          WHERE sub_department = ? ", cond_sub_department)]
  list_list_prices_cond = []
  for product in list_products_cond:
    prod_condition = (product,)
    list_prices_cond_temp = [row for row in cursor.execute("SELECT DISTINCT product_total_price, period\
                                                            FROM auchan_velizy WHERE product_title = ?\
                                                            ORDER BY period ASC", prod_condition)]
  ...
  for list_product_prices in list_list_prices_cond:
    ...
    price_series = [dico_period_prices.get(i, np.nan) for i in range(104)] # must give a number of periods
...
# TODO: Split stat des work betweek SQL/Python
# TODO: Global stats on price last digits, price change timing/frequency/magnitude
# TODO: Graph price series of volatile prices (withing subdpt or dpt first...)
# TODO: Account for price changes: brand, product characteristics... etc (to be created: regex etc.)

"""
# ALTERNATIVELY FOR 1/ AND 2/
# get list of (title, price, period) tuples ranked in title, period ascending order (on sub_dpt_cond)
list_products_periods = [row for row in cursor.execute("SELECT DISTINCT product_title, product_total_price, period\
