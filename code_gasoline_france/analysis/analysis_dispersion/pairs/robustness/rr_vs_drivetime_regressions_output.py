...
# LOAD PAIR DISTANCE (OSRM)

df_pair_distance = pd.read_csv(os.path.join(path_dir_built_csv,
...
# ADD OSRM DISTANCE AND CHOOSE DISTANCE VAR
df_pairs = pd.merge(df_pairs,
                    df_pair_distance[['id_1', 'id_2', 'osrm_dist', 'osrm_time']],
                    on = ['id_1', 'id_2'],
                    how = 'left')
col_distance = 'osrm_dist'
print(u'Distance variable: {:s}'.format(col_distance))

# DEFINE SAME CORNER VARIABLES
df_pairs['sc_500'] = 0
df_pairs.loc[df_pairs[col_distance] <= 0.5, 'sc_500'] = 1
df_pairs['sc_750'] = 0
df_pairs.loc[df_pairs[col_distance] <= 0.75, 'sc_750'] = 1
df_pairs['sc_1000'] = 0
df_pairs.loc[df_pairs[col_distance] <= 1, 'sc_1000'] = 1
df_pairs['sc_1500'] = 0
df_pairs.loc[df_pairs[col_distance] <= 1, 'sc_1500'] = 1
# Consider only pairs within this distance in analysis
max_distance = 5
...
for df_ppd_title, df_ppd_reg in ls_loop_df_ppd_regs:
  df_ppd_reg = df_ppd_reg[(df_ppd_reg[col_distance] <= max_distance)].copy()
