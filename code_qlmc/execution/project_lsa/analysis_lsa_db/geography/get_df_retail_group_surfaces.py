...
for retail_group in ls_rgs:
  ...
  for i, row in df_com.iterrows():
    ...
    df_lsa_rg['dist'] = compute_distance_ar(df_lsa_rg['Latitude'],
                                            ...
                                            df_lsa_rg['lat_com'],
                                            df_lsa_rg['lng_com'])
    df_lsa_rg['wgt_surf'] = np.exp(-df_lsa_rg['dist']/10) * df_lsa_rg['Surf Vente']
    row_res = [i, df_lsa_rg['wgt_surf'].sum()]
    # df_com.loc[i, 'avail_surf'] = df_lsa_rg['wgt_surf'].sum()
    
    #for store_type in ['H', 'X', 'S']:
    #  min_dist = df_lsa_rg['dist'][df_lsa_rg['Type_alt'] == '%s' %store_type].min()
  ...
  df_com_rg = pd.DataFrame(ls_rows_res, columns = ['code_insee', 'avail_surf'])
  df_com_rg.set_index('code_insee', inplace = True)
  df_com['avail_surf_%s' %retail_group] = df_com_rg['avail_surf']

# SURFACE IN EACH COMMUNE BY BRAND
for retail_group in ls_rgs:
  df_lsa_rg = df_lsa[df_lsa['Groupe'] == retail_group].copy()
  se_rg_com_surf = df_lsa_rg[['Code INSEE ardt', 'Surf Vente']]\
                     .groupby('Code INSEE ardt').agg(np.sum)['Surf Vente']
  df_com['surf_%s' %retail_group] = se_rg_com_surf
