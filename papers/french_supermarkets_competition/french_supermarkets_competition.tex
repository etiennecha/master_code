...
Store and market characteristics are found to account for a small share of the variance in store indexes. In particular, Leclerc does not appear to be significantly less price competitive relative to competitors once the size of its stors and their location is taken into account. The affiliation of a store appears to be a strong determinant of its overall price level, which is consistent with the relative strong intra-chain price concentration previously obtained and other studies on retail chain prices. \cite{HOS08} and \cite{CHA16} observe similar results with gas station prices respectively in the US and in France (even though gas station chains do not follow uniform pricing policies).

\subsection{National level dispersion}

Product price dispersion is measured at the national level, with a view to get some measure of potential heterogeneity among products. Given previous observations on chain pricing policies, only products where X observations are available from at least X different supermarket chains are taken into account. For lack of precise product characteristics and information on sales, product section and families are used to investigate differences in observed product price dispersion. Table~\label{tab:reg_nat_dispersion} reports the results of the regression:

\begin{align}
\text{Product dispersion} = \text{Product average price} + \text{Product section} + \epsilon
\label{reg_store_indexes}
\end{align}
