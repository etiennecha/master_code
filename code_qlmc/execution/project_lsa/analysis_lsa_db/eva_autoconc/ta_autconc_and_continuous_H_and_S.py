...
for type_store, type_comp, dist_comp in ls_h_and_s:
  ...
  for row_ind, row in df_lsa_type.iterrows():
    ...
    ls_rows_des_comp.append((row_ind, store_share, groupe_share, hhi,
                             ac_nb_stores, ac_nb_comp, ac_store_share, ac_groupe_share, ac_hhi))
  
  df_des_comp = pd.DataFrame(ls_rows_des_comp,
                              columns = ['index', 'store_share', 'group_share', 'hhi',
                                        'ac_nb_store', 'ac_nb_comp', 'ac_store_share',
                                        'ac_group_share', 'ac_hhi'])
  ...
  df_lsa_type.to_csv(os.path.join(path_dir_built_csv,
                                  'df_comp_%s_ac_vs_cont.csv' %type_store),
                     encoding = 'latin-1',
                     float_format ='%.3f',
                     date_format='%Y%m%d',
                     index = False)
