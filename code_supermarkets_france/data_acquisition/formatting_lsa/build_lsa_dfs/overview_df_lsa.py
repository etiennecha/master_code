...
print u'\nShare of each group by region (nb stores):'
...
print u'\nShare (or "weight") of each region within each group (nb stores):'
df_reg_2 = df_reg_2.T
...
print df_reg_2[ls_reg_disp].to_string()
...
print u'\nShare of each group by region (surface):'
...
print df_reg_surf[ls_reg_disp].to_string()
#print 'TOTAL &', ' & '.join(map(lambda x: '{:4.0f}'.format(x),
#                            df_reg_surf[ls_reg_disp].sum(axis = 0).values)), '\\\\'

print u'\nShare (or "weight") of each region within each group (surface):'
...
print df_reg_surf_2[ls_reg_disp].to_string()
...
print u'\nRegional CRs and HHI:'
print df_reg_cr[ls_disp].to_string(formatters = dict_formatters)
