...
for row_ind, row in df_lsa_sub.iterrows():
  ...
  df_lsa_market = df_lsa_sub_temp[df_lsa_sub_temp['dist'] <= 25].copy()
  ...
  nb_stores = len(df_lsa_market) - 1 # exclude reference store (even if no group)
  nb_comp = len(df_lsa_market_comp) # reference store already excluded (even if no group)
  store_share = row['Surf Vente'] / df_lsa_market['Surf Vente'].sum()
  groupe_share = df_lsa_market_groupe['Surf Vente'].sum() /\
  ...
  ls_rows_des_comp.append((row_ind, nb_stores, nb_comp, store_share, groupe_share, hhi))

df_des_comp = pd.DataFrame(ls_rows_des_comp,
                            columns = ['index', 'nb_stores', 'nb_comp',
...
ls_disp_lsa_comp = ls_disp_lsa +\
                   ['nb_stores', 'nb_comp', 'store_share', 'group_share', 'hhi']
