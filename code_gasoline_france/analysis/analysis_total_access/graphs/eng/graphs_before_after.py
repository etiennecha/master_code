...
dir_graphs = 'eng_version'
alpha_1 = 0.2
alpha_2 = 0.4
str_xlabel = u'Price (euro/liter)'
str_ylabel = u'Nb. of gas stations'
...
ax.hist(df_prices_ttc[ls_total_nta_ids].iloc[0].values,
        bins, alpha=alpha_1, label=u'"Total" (not to be rebranded)', color = 'g')
ax.hist(df_prices_ttc[ls_total_ta_ids].iloc[0].values,
        bins, alpha=alpha_2, label=u'"Total" to be rebranded "Total Access"', color = 'b')
...
ax.hist(df_prices_ttc[ls_total_nta_ids].iloc[-1].values,
        bins, alpha=alpha_1, label=u'"Total"', color = 'g')
ax.hist(df_prices_ttc[ls_total_ta_ids].iloc[-1].values,
        bins, alpha=alpha_2, label=u'"Total Access" previously "Total"', color = 'b')
...
ax.hist(df_prices_ttc[ls_nonaffected_ids].iloc[0].values,
        bins, alpha=alpha_1, label=u'Gas stations not to be impacted', color = 'g')
ax.hist(df_prices_ttc[ls_treated_comp_ids].iloc[0].values,
        bins, alpha=alpha_2, label=u'Gas stations to be impacted', color = 'b')
plt.xticks(np.linspace(1.20, 1.60, 9))
plt.xlim(1.20, 1.60)
plt.xlabel(str_xlabel)
plt.ylabel(str_ylabel)
...
ax.hist(df_prices_ttc[ls_nonaffected_ids].iloc[-1].values,
        bins, alpha=alpha_1, label=u'Non-impacted gas stations', color = 'g')
ax.hist(df_prices_ttc[ls_treated_comp_ids].iloc[-1].values,
        bins, alpha=alpha_2, label=u'Impacted gas stations', color = 'b')
plt.xticks(np.linspace(1.00, 1.50, 11))
plt.xlim(1.00, 1.50)
plt.xlabel(str_xlabel)
plt.ylabel(str_ylabel)
