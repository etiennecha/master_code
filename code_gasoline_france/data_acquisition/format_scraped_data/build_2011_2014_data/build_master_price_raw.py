        ...
        if obs_id in master['ids']:
          ...
          for (tuple_index, var_name) in dict_var_types['varying']:
            if obs_tuple[tuple_index] != master['dict_info'][obs_id][var_name][-1][0]:
              master['dict_info'][obs_id][var_name].append((obs_tuple[tuple_index],
                                                            day_index))
        else:
...
end_date = date(2014, 12, 4)

#DIESEL MASTER
path_in = os.path.join(path_dir_source_prices,
                       'diesel_standardized_tuple_lists')
