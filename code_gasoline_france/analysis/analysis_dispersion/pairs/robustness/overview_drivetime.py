...
for col in ls_col_overview:
  ...
  for distance in ls_distances:
    ls_se_temp.append(df_pair_temp[df_pair_temp[col_distance] <= distance]\
                                  [col].describe())
...
for dist_lim in [10, 5, 3, 1, 0.5]:
  ...
  for title, df_pair_disp in ls_loop_pair_disp:
    df_pair_disp_sub = df_pair_disp[df_pair_disp[col_distance] <= dist_lim]
...
for dist_lim in [0.5, 1, 3, 5, 10]:
  df_temp = df_pair_comp[df_pair_comp[col_distance] <= dist_lim]
...
df_pairs = df_pair_comp[df_pair_comp[col_distance] <= 5].copy()
