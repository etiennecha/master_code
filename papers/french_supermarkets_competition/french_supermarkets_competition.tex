...
From a methodological point a view, it must be noted that the maximum values observed at the store level must be interpreted with caution, as the mode can result from a few stores always setting the same price. The analysis can be refined by discarding price modes which are not followed by a large enough proportion of the chain stores. Robustness checks performed with thresholds of 33\% and 50\% confirm that Geant Casino and Leclerc stand out in terms of price concentration.
