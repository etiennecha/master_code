...
print sm.OLS(rank_reversals, sm.add_constant(distance), missing = "drop").fit().summary(yname='rank_reversals', xname = ['constant', 'distance'])
print sm.OLS(spread_std, sm.add_constant(distance), missing = "drop").fit().summary(yname='spread_std', xname = ['constant', 'distance'])
...
# WITH PANDAS
pd_master_pd = pd.DataFrame(matrix_master_pd, columns=['nb_competitors', 'price', 'std_prices', 'coeff_var_prices', 'range_prices', 'gain_search'])
...
gain_search = matrix_master_pd[:,5]
print '\n REGRESSIONS OF MARKET PRICE DISPERSION ON NB COMPETITORS AND PRICE \n'
print sm.OLS(std_prices, sm.add_constant(nb_competitors)).fit().summary(yname='std_prices', xname = ['constant', 'nb_competitors'])
print sm.OLS(std_prices, sm.add_constant(nb_competitors_and_price)).fit().summary(yname='std_prices', xname = ['constant', 'nb_competitors', 'price'])
print sm.OLS(coeff_var_prices, sm.add_constant(nb_competitors)).fit().summary(yname='coeff_var_prices', xname = ['constant', 'nb_competitors'])
print sm.OLS(coeff_var_prices, sm.add_constant(nb_competitors_and_price)).fit().summary(yname='coeff_var_prices', xname = ['constant', 'nb_competitors', 'price'])
print sm.OLS(range_prices, sm.add_constant(nb_competitors)).fit().summary(yname='range_prices', xname = ['constant', 'nb_competitors'])
print sm.OLS(range_prices, sm.add_constant(nb_competitors_and_price)).fit().summary(yname='range_prices', xname = ['constant', 'nb_competitors', 'price'])
print sm.OLS(gain_search, sm.add_constant(nb_competitors)).fit().summary(yname='gain_search', xname = ['constant', 'nb_competitors'])
print sm.OLS(gain_search, sm.add_constant(nb_competitors_and_price)).fit().summary(yname='gain_search', xname = ['constant', 'nb_competitors', 'price'])
"""
...
# CLEANING WITH PANDAS : ONE PERIOD

period_ind = 0
index = master_price_gas_stations['list_ids']
list_station_prices = [list_prices[period_ind] for list_prices in master_price_gas_stations['list_indiv_prices']]
list_station_brands = [dico_brands[get_str_no_accent(get_id_list_field_results(id, 'brand_station', master_price_gas_stations)[period_ind]).upper()][1] for id in master_price_gas_stations['list_ids']]
list_station_type = [dico_brands[get_str_no_accent(get_id_list_field_results(id, 'brand_station', master_price_gas_stations)[period_ind]).upper()][2] for id in master_price_gas_stations['list_ids']]
dico_period = {'price' : list_station_prices,
               'brand' : list_station_brands,
			         'type' : list_station_type}
pd_period_prices = pd.DataFrame(dico_period, index = index)
pd_period_prices['zip_code'] =  pd_period_prices.index.map(lambda x: x[:-5])
y,X = patsy.dmatrices('price ~ brand', pd_period_prices, return_type='dataframe')
print sm.OLS(y, X, missing = 'drop').fit().summary()
y,X = patsy.dmatrices('price ~ brand + C(zip_code)', pd_period_prices, return_type='dataframe')
print sm.OLS(y, X, missing = 'drop').fit().summary()
# then predict price ... repeat for each period... finally convert to array then use to list...
...
quantiles = qcut(brand_prices, [0, .25, .5, .75, 1])
print pd.value_counts(quantiles)

# generate figure with price histograms per brand
list_of_brands = np.unique(pd_period_prices['brand'])
# should take sqrt of list length rounded above to unit

fig, axes = plt.subplots(nrows=5, ncols=5)
list_axes = [j for i in axes for j in i][:len(list_of_brands)]
for i, ax in enumerate(list_axes):
  brand_prices = pd_period_prices['price'][pd_period_prices['brand'] == list_of_brands[i]]
  test = ax.hist(brand_prices, bins = 20)
  test = ax.set_title(list_of_brands[i], fontsize=8)
plt.rcParams['font.size'] = 8
#plt.tight_layout()
plt.show()
plt.rcParams['font.size'] = 12

# CLEANING WITH PANDAS : PANEL DATA (IN PROGRESS)
# create a dict: keys = station ids, contents = DataFrame with gas station prices etc
"""
data = {'Item1' : pd.DataFrame(np.random.randn(4,3)), 'Item2' : pd.DataFrame(np.random.randn(4,2))}
lol = pd.Panel(data)
"""
dico_panel_data_master = {}
for i, id in enumerate(master_price_gas_stations['list_ids']):
  dico_station = {'price' : master_price_gas_stations['list_indiv_prices'][i],
                  'brand' : map(lambda brand_name: dico_brands[get_str_no_accent(brand_name).upper()][1], get_id_list_field_results(id, 'brand_station', master_price_gas_stations))}  
  dico_panel_data_master[id] = pd.DataFrame(dico_station, index = master_price_gas_stations['list_dates'])
pd_panel_data_master = pd.Panel(dico_panel_data_master)
