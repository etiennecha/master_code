...
dict_docs = {'generaliste' : ('medecin-generaliste', 'Médecin généraliste'),
             'ophtalmo'    : ('ophtalmologiste', 'Ophtalmologiste'),
             'gyneco'      : ('gynecologue-medical', 'Gynécologue médical')}

ps_profession = u'ophtalmologiste'
ps_profession_label = u'ophtalmologiste'
ps_localisation = u'75'

def get_ls_docs(path_folder, tup_profession, location):
  ...
  dict_search_params = {'type':'ps',
                        'ps_nom':'',
                        'es_nom':'',
                        'ps_profession': tup_profession[0],
                        'ps_profession_label': tup_profession[1],
                        'es_specialite':'',
                        'es_specialite_label':'',
                        'ps_acte':'',
                        'ps_acte_label':'',
  ...
  return ls_ls_physicians

def get_dict_docs(path_folder, tup_profession, location, ls_ls_physicians):
  path_dict_physicians = path_folder + r'\dict_%s_%s' %(tup_profession[0], location)
  if os.path.exists(path_dict_physicians):
    dict_physicians = dec_json(path_dict_physicians)
  else:
    dict_physicians = {}
  for ls_physician in ls_ls_physicians:
    ...
    if id_match and id_match.group(1) not in dict_physicians:
      ...
      dict_physicians[id_physician] = [ls_name_address,
                                       ...
                                       ls_all_actes]
      time.sleep(2)
  return dict_physicians

#path = r'C:\Users\etna\Desktop\Etienne_work\Data'
path = r'\\ulysse\users\echamayou\Bureau\Etienne_work\Data'
folder_physicians = r'\data_ameli'
path_folder_physicians = r'%s\%s' %(path, folder_physicians)
tup_profession, location = dict_docs['ophtalmo'], '92'
ls_ls_physicians = get_ls_docs(path_folder_physicians, tup_profession, location) 
dict_physicians = get_dict_docs(path_folder_physicians, tup_profession, location, ls_ls_physicians[0:100])
 
path_ls_ls_physicians = path_folder_physicians + r'\ls_ls_%s_%s' %(tup_profession[0], location) 
enc_json(ls_ls_physicians, path_ls_ls_physicians)

path_dict_physicians = path_folder_physicians + r'\dict_%s_%s' %(tup_profession[0], location) 
enc_json(dict_physicians, path_dict_physicians)
