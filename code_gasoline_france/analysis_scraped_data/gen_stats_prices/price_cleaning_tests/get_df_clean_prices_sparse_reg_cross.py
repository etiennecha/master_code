...
# caution: content of res[0]
# sorted according to param names (for id, then date)

# GET FIXED EFFECTS
se_res_dates = df_final['date'].drop_duplicates()
se_res_dates.sort(inplace = True)
se_res_ids = df_final['id'].drop_duplicates()
se_res_ids.sort(inplace = True)
se_res_fe = pd.concat([se_res_dates, se_res_ids], axis = 0, ignore_index = True)
df_res = pd.DataFrame(zip(res[0], res[-1]),
                      columns = ['beta', 'invtAA'],
                      index = se_res_fe.values)
df_res['std'] = np.sqrt(df_res['invtAA'] * res[3]**2 /\
                          (sparse_mat_id_dates.shape[0] - len(res[0])))

df_res['t_stat'] = df_res['beta'] / df_res['std']
print df_res.to_string()
#df_res['t_stat'] = df_res['beta'] / np.sqrt(df_res['invtAA'])
#print df_res.to_string()

## COMPUTE CLEANED PRICES
...
#ind_ids_chge = df_margin_chge[~pd.isnull(df_margin_chge['date'])].index
...
#  if id_station in df_prices.columns:
